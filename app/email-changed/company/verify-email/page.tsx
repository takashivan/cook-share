...
export default function CompanyVerifyEmailPage() {
  ...
  const { user, isLoading, changeEmail } = useCompanyAuth();
  ...
  const handleResendEmail = async () => {
    try {
      ...
      await changeEmail(user.pending_email);
      setShowSuccess(true);
      toast({
        title: "確認メールを再送信しました",
  ...
  const handleUpdateEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(error.message || "メールアドレスの更新に失敗しました");
    } finally {
      setIsUpdating(false);
    }
  };
            ...
            <CardHeader>
              ...
              <motion.div
                ...
                <CardDescription className="text-center mt-2 text-gray-600">
                  {user.pending_email} に確認メールを送信しました。
                  <br />
                  メールに記載されたリンクをクリックして、メールアドレスを認証してください。
                  <br />
                  認証が完了次第、新しいメールアドレスに切り替わります。
                </CardDescription>
              </motion.div>
            </CardHeader>
            <CardContent>
